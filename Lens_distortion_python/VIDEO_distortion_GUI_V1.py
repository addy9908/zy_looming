...
class VideoCorrectorApp:
    ...
    def auto_find_files(self):
        ...
        # --- MODIFIED REGEX: Now captures both base name and date ---
        # It looks for (group 1: anything) followed by _ (group 2: YYYY-MM-DD) followed by T
        match = re.match(r'^(.*?)_(\d{4}-\d{2}-\d{2})T', video_filename)
        
        if not match:
            messagebox.showerror("Error", "Could not determine base name and date from filename.\nExpected pattern: [BaseName]_[YYYY-MM-DD]T[...].avi")
            return
            
        base_name = match.group(1)
        date_string = match.group(2) # e.g., "2026-03-05"
        
        self.status_var.set(f"Searching for files with base: {base_name} and date: {date_string}")
        
        found_cam, found_looming = [], []
        for f in os.listdir(directory):
            # --- MODIFIED CONDITIONS: Now checks for date string as well ---
        ...
        # Load Cam file
        if len(found_cam) == 1:
            cam_path = os.path.join(directory, found_cam[0])
            ...
            messagebox.showwarning("Auto-Find", f"Found {len(found_cam)} possible Cam files. Please load manually.")
        
        # Load Looming file
        if len(found_looming) == 1:
            looming_path = os.path.join(directory, found_looming[0])
            ...
            messagebox.showwarning("Auto-Find", f"Found {len(found_looming)} possible Looming files. Please load manually.")
        
        if cam_loaded and looming_loaded:
            self.status_var.set("Successfully auto-loaded companion files.")
        else:
            self.status_var.set("Auto-find complete. Check for manual loading.")
    ...
    def start_correction_process(self):
        ...
        if self.correction_params:
            ...
            if user_choice: self.process_video(self.correction_params, start_frame_num); return
        self.run_calibration_workflow(start_frame_num)

    def run_calibration_workflow(self, start_frame_num):
        global points
        self.status_var.set(f"Starting calibration..."); self.root.update()
        ...
        self.process_video(self.correction_params, start_frame_num)

    def process_video(self, params, start_frame_num):
        k1_final, cx_final, cy_final, M_final, output_dims, new_cam_matrix_final = params
        cap_check = cv2.VideoCapture(self.video_path); h, w = cap_check.get(cv2.CAP_PROP_FRAME_HEIGHT), cap_check.get(cv2.CAP_PROP_FRAME_WIDTH); cap_check.release()
        ...
        batch_cap = cv2.VideoCapture(self.video_path); batch_cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame_num)
    ...
    def log_analysis_results(self, output_video_path, params):
        ...
        # --- THE FIX: Flatten the arrays into a simple, unambiguous string ---
        ...
        log_data = {
            ...
            "lens_cx": f"{cx:.2f}",
            "lens_cy": f"{cy:.2f}",
            "M_matrix_flat": M_flat_str, # Use the flat string
            "output_dims": f"{dims[0]},{dims[1]}",
            "new_cam_matrix_flat": new_cam_flat_str # Use the flat string
        }
