...
class LoomingAnalyzer(QMainWindow):
    ...
    def initUI(self):
        ...
        parts = ["mouse_center", "nose", "left_ear", "right_ear", "neck", "mid_back", "tail_base",  
                 "left_shoulder", "left_midside", "left_hip", "right_shoulder", 
                 "right_shouder", "right_midside", "right_hip", "head_midpoint"]
        ...
        for txt in [self.txt_fps, self.txt_px_cm, self.txt_vid_w, self.txt_vid_h, self.txt_likelihood]:
            txt.editingFinished.connect(self.recalculate_data)
    ...
    def load_files(self, file_type):
        ...
        if files:
            ...
            for f in self.files:
                ...
                self.file_data[f] = {
                    'type': file_type,
                    'mouse_id': os.path.basename(f).split('_')[2],
                    'photostim': 'OFF',
                    'freeze_start': None,
                    'freeze_end': None,
                    'shelter_time': None,
    ...
    def parse_file(self, filepath):
        ...
        try:
            ...
            if d['type'] == 'dlc':
                ...
                self.btn_calib.setEnabled(False)
                df = pd.read_csv(filepath) # Already parsed saved data
    ...
    def parse_dlc(self, filepath, meta):
        ...
        x_parts, y_parts = [], []
        bps = [p for p in self.combo_bp.get_checked_items() if p in df_h5.columns.get_level_values('bodyparts')]
        if not bps: bps = [df_h5.columns.get_level_values('bodyparts')[0]]
        ...
        x_px_filled = x_px_raw.ffill()
        y_px_filled = y_px_raw.ffill()
